...
export const getApiFormatsForProvider = (provider: string): ApiFormat[] => {
  const formats = getApiFormatsForProviderFromConfigs(provider, protocolConfigs);
  const providerConfig = PROVIDER_CONFIGS[provider];
  if (
    providerConfig?.channelTypes.some((channelType) => IMAGE_GENERATION_CHANNEL_TYPES.has(channelType)) &&
    !formats.includes(OPENAI_IMAGE_GENERATION)
  ) {
